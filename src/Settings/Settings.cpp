...
const char DEFAULT_WALLET_FILE_NAME[] = "hongbaowallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "18.217.1.220:3333";
  m_defaultPoolList << "18.217.1.220:5555"; 
  m_defaultPoolList << "18.217.1.220:7777";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("hongbaowallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hongbaowallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hongbaowallet.desktop");
  ...
  res = autorunSettings.contains("HongbaoWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("HongbaoWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.hongbao.hongbaowallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Hongbao Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("HongbaoWallet", appPath);
    } else {
      autorunSettings.remove("HongbaoWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\hongbao", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:hongbao");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\hongbao\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\hongbao\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("hongbaowallet.cfg"));
