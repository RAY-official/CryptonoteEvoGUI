...
const char BYTECOIN_URI_SCHEME_NAME[] = "hongbao";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("hongbaowallet");
  setApplicationVersion(Settings::instance().getVersion());
  setQuitOnLastWindowClosed(false);
  setStyle(QStyleFactory::create("Hongbao"));
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("hongbaowallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Hongbao wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Hongbao wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have hongbaod with non-default RPC port, you should terminate it and relaunch BytecoinWallet\n"
      "or\n"
      "Set the Local deamon required port in HongbaoWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
