...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_hongbaoDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfHongbaoBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 hongbaoHeight;
  if (!checkIfBlockchainOutdated(currentHeight, hongbaoHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(hongbaoHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfHongbaoBlockchainExists() const {
  return m_hongbaoDir.exists() && m_hongbaoDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_hongbaoDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::hongbaoBlockchainInfo() const {
  return QFileInfo(m_hongbaoDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_hongbaoDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_hongbaoDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
